...
def test_an_explicit_venue_still_wins_over_a_newer_other_venue():
    """The documented preference, unchanged. Stowaway's own July price beats
    Harry Gatos' August one when Stowaway is what was asked for."""
    cs = CostSeries([
        _obs("x", date(2026, 7, 1), "1.00", "stowaway"),
        _obs("x", date(2026, 8, 12), "9.99", "harry_gatos"),
    ], purchasable_to_ingredient={})
    assert cs.as_of("x", ON, venue="stowaway").cost_per_unit == Decimal("1.00")
